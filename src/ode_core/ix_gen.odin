...
// ix_gen -- index + generation.
    ...
    // Same as free_id, minus the bounds/already-freed/not-found checks — for a
    // caller that has already proven `id` correct (in-bounds, not expired)
    // against this same `items` array moments earlier.
    ix_gen_factory__free_id_trusted :: #force_inline proc (self: ^Ix_Gen_Factory, id: ix_gen) #no_bounds_check {
        when VALIDATIONS {
            assert(id.ix >= 0 && id.ix < self.cap)
            assert(self.items[id.ix] == id)
            assert(self.freed_count < self.cap)
        }

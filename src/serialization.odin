/*
    ...
    generations, so saved entity_ids stay valid after load), components and
    relations round-trip. Database.eid_to_table is derived data and is
    rebuilt on load from each table's own rows, not stored separately.
...
// Format
    ...
    @(private)
    Snap_Table_Column_Header :: struct #packed {
        comp_size:  i64,
        comp_align: i64,
        name_len:   i64,
    }
...
// Table helpers
    ...
    database__snapshot_check_not_paused :: proc(self: ^Database, check_holes: bool) -> Error {
        ...
        for table in self.tables.items {
            ...
            if check_holes && table.holes_count > 0 {
    ...
    database__serialized_size :: proc(self: ^Database) -> (size: int, err: Error) {
        ...
        for table in self.tables.items {
            ...
            for col in table.columns {
                size += size_of(Snap_Table_Column_Header)
...
// Serialize
    ...
    database__serialize :: proc(self: ^Database, buf: []byte, allow_non_pod := false) -> (written: int, err: Error) {
        ...
        if !allow_non_pod {
            for table in self.tables.items {
                if table == nil do continue
                for col in table.columns {
                    if !snapshot__type_is_pod(col.type_info) do return 0, API_Error.Snapshot_Component_Not_POD
                }
            }
        }
    ...
    @(private)
    table__snapshot_write :: proc(table: ^Table, w: ^Snap_Writer) {
        n := table.len
        ...
        th := Snap_Table_Header{
            ...
            cap          = i64(table.cap),
            len          = i64(n),
            column_count = i64(len(table.columns)),
        }
        snap_writer__write(w, &th, size_of(th))

        for col in table.columns {
            cih := Snap_Table_Column_Header{
        ...
        snap_writer__write(w, raw_data(table.rid_to_eid), n * size_of(entity_id))
        snap_writer__pad8(w)
...
// Deserialize
    ...
    database__deserialize :: proc(self: ^Database, data: []byte) -> Error {
        ...
        for section_ix in 0..<int(hdr.section_count) {
            ...
            if n > table.cap do return API_Error.Snapshot_Capacity_Too_Small

            if int(th.column_count) != len(table.columns) do return API_Error.Snapshot_Schema_Mismatch
        ...
        for _ in 0..<int(hdr.section_count) {
            th: Snap_Table_Header
            snap_reader__read(&r, &th, size_of(th)) or_return
            table__snapshot_apply(self.tables.items[int(th.table_id)], &th, &r) or_return
        }
    ...
    @(private)
    table__snapshot_apply :: proc(table: ^Table, th: ^Snap_Table_Header, r: ^Snap_Reader) -> Error {
        n := int(th.len)
        db := table.db
        ...
        #no_bounds_check for rid in 0..<n {
            eid := eids[rid]
            table.eid_to_rid[eid.ix] = u32(rid)
            db.eid_to_table[eid.ix] = table
        }

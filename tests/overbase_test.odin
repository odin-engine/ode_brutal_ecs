...
// Overbase
    ...
    overbase_init_from_overbase_caps__test :: proc(t: ^testing.T) {
        ...
        db: ecs.Database
        defer ecs.terminate(&db)
        // tables_cap must be > 1 (pre-existing constraint, unrelated to this
        // change) — 2 is the smallest legal value.
        testing.expect(t, ecs.init_from_overbase(&db, &ob, tables_cap = 2) == nil)

        t1: ecs.Table
        defer ecs.table__terminate(&t1)
        t2: ecs.Table
        defer ecs.table__terminate(&t2)
        testing.expect(t, ecs.table__init(&t1, &db, 10, {Ob_Position}) == nil)
        testing.expect(t, ecs.table__init(&t2, &db, 10, {Ob_Sprite}) == nil)

        // tables_cap == 2 already used up by t1/t2 — a 3rd table grows
        // db.tables (doubling) instead of failing.
        t3: ecs.Table
        defer ecs.table__terminate(&t3)
        testing.expect(t, ecs.table__init(&t3, &db, 10, {Ob_Position}) == nil)
    ...
    overbase_destroy_cleans_all_databases__test :: proc(t: ^testing.T) {
        ...
        //
            ...
            perr := ecs.table__add_entity(&positions, robot)
            testing.expect(t, perr == nil)
            serr := ecs.table__add_entity(&sprites, robot)
    ...
    overbase_destroy_children_cascades_across_databases__test :: proc(t: ^testing.T) {
        ...
        //
            ...
            perr := ecs.table__add_entity(&positions, child)
            testing.expect(t, perr == nil)
            serr := ecs.table__add_entity(&sprites, child)
            testing.expect(t, serr == nil)

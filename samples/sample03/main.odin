/*
    ...
    components — they live independently of any table.
*/

package ode_ecs_sample03
...
main :: proc() {
    ...
    //
        ...
        for eid in ([]ecs.entity_id{ship, turret1, turret2}) {
            err = ecs.add_entity(&transforms, eid)
            if err != nil { report_error(err); return }
            ecs.set_component(&transforms, eid, Transform{ 10, 20 })
        }
    ...
    //
        ...
        for child in children {
            t := ecs.get_component(&transforms, child, Transform)
            t.x += 5

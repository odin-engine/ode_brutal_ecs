...
// databases must coincide. An entity belongs to at most one table, so
// "positions" and "flags" hold disjoint entity sets here.
//
World :: struct {
    db: ecs.Database,
    positions: ecs.Table,
...
world__init :: proc(w: ^World, allocator: mem.Allocator) -> ecs.Error {
    ...
    ecs.table_init(&w.positions, &w.db, 100, {Position}) or_return
    ecs.table_init(&w.flags, &w.db, 100, {Marker}) or_return
    return nil
}
...
main :: proc() {
    ...
    //
        eids: [7]ecs.entity_id
        for i in 0..<5 {
            eids[i], err = ecs.create_entity(&a.db)
            if err != nil { report_error(err); return }

            err = ecs.add_entity(&a.positions, eids[i])
            if err != nil { report_error(err); return }
            ecs.set_component(&a.positions, eids[i], Position{ x = f32(i * 10), y = f32(i) })
        }
        for i in 5..<7 {
            eids[i], err = ecs.create_entity(&a.db)
            if err != nil { report_error(err); return }
            err = ecs.add_entity(&a.flags, eids[i])
            if err != nil { report_error(err); return }
        }
        ...
        fmt.println("World A:", ecs.entities_len(&a.db), "entities,",
            ecs.table_len(&a.positions), "positions,", ecs.table_len(&a.flags), "flags")
    ...
    //
        ...
        pos_b := ecs.get_component(&b.positions, eids[2], Position)
        fmt.println("World B (from buffer): entity 2 position =", pos_b^,
            ", flagged =", ecs.has_component(&b.flags, eids[5]))
    ...
    //
        path :: "sample04.snap" // cwd-relative; removed below
        ...
        pos_c := ecs.get_component(&c.positions, eids[2], Position)
        fmt.println()
        fmt.println("World C (from", path, "): entity 2 position =", pos_c^,
            ", flagged =", ecs.has_component(&c.flags, eids[5]))
}

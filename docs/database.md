...
A **`Database`** is the "world" object of ODE_BRUTAL_ECS — it owns entities, component tables (`Table`), and (optionally) parent/child relations. Other ECS libraries call this concept a *World* or *Scene*. A single game can use as many independent databases as it wants; they share nothing.

All memory is preallocated when the database (and its tables) are initialized. Nothing reallocates during the game loop.

See also: [Overbase](overbase.md) · [Table](table.md) · [Relations](relations.md)
...
You can pass a custom allocator; every table attached to this database will use it too:

```odin
ecs.init(&my_ecs, entities_cap = 100, allocator = my_allocator)
```

`terminate` frees everything owned by the database, including all attached tables — you do not need to terminate them individually. After `terminate` the struct is back in the `Not_Initialized` state and can be re-`init`-ed.
...
An entity is just an ID (`ecs.entity_id`) — a `bit_field` packing an index (`ix`, 32 bits) and a generation (`gen`, 32 bits). All entity data lives in [`Table`](table.md) columns.
...
`destroy_entity` automatically removes the entity's row from the one `Table` it belongs to (`Database.eid_to_table` is a direct per-entity pointer, so this is a single O(1) lookup, not a scan). If a [Relations_Table](relations.md) exists, the entity is also unlinked from its parent and its children are orphaned — or destroyed too, with the optional flag:
...
One limit to know: the generation is 32 bits, so it wraps after 4,294,967,296 create/destroy cycles of the *same index*. A stale ID held across exactly that many recycles collides with the new live ID and reads as valid again (the classic ABA case of generational indexes). In practice, this needs a saved ID to survive 4.29 billion reuses of one slot, which is astronomically unlikely for any real workload — but don't cache entity IDs across arbitrarily long spans without re-validating them by your own means (or re-looking them up) if your entity churn is extreme.
...
`ecs.clear(&my_ecs)` wipes all data — every table, relations and all entities — but keeps every object initialized with its capacity intact. Generations are bumped so entity IDs held across the clear are detected as expired. Useful for "restart level" without re-initializing anything.
...
While paused, removals clear components **in place**, leaving holes: no row moves, component pointers stay stable. `get_entity` for a hole returns an ID with `ix == ecs.DELETED_INDEX`, and `table_len` keeps reporting the full row span including holes. `resume_packing` packs every table that accumulated holes. You can also call `ecs.pack(&table)` on an individual table mid-pause (holes do not free capacity until packed).

### Pause scope: Database or Table

`pause_packing`/`resume_packing`/`pack` work at two levels:

- **Database** (`ecs.pause_packing(&my_ecs)`, above) — pauses every table in the database.
- **Table** (`ecs.pause_packing(&monsters)`) — pauses just that one table, independent of the database-wide flag.

Use table-level pause to isolate one table from a concurrent database-wide pause/resume — e.g. one thread mutates/iterates `&monsters` under a table-level pause while another thread runs a database-wide `pause_packing`/`resume_packing` cycle over unrelated tables. The scopes compose (OR together): a database-wide `resume_packing` still packs every table, but does not forcibly clear a table's own independent pause.

## Utilities

```odin
ecs.is_valid(&my_ecs)       // is the database initialized and in a Normal state?
ecs.memory_usage(&my_ecs)   // total bytes used by the database and all its tables
```
...
setup :: proc(db: ^ecs.Database) -> ecs.Error {
    ...
    ecs.table_init(&units, db, 1000, component_types = {Position}) or_return
    return nil
}
```
...
| `ECS_TABLES_CAP` | `16` | Initial preallocation for `Database.tables` (an auto-growing sparse array — not a hard ceiling on table or component-type count) |
| `ECS_PAIR_TABLES_CAP` | `8` | Initial preallocation for attached `Pair_Table`s |
| `ECS_SYNC_ENABLED` | `false` | Compiles in delta-change replication (`Sync_Channel`/`Sync_Decoder`) |
| `ECS_SYNC_CHANNELS_CAP` | `8` | Max sync channels a single `Table` can be registered to |

There is no compile-time ceiling on the number of tables or component types — `Database.tables` grows as needed.

...
See also: [Database](database.md) · [Table](table.md)
...
`databases_cap` is the maximum number of Databases that can attach to this Overbase — like every other capacity in ODE_BRUTAL_ECS, it's preallocated up front (defaults to `1`, the size needed for the common single-owner case). `init_from_overbase` fails with `oc.Core_Error.Container_Is_Full` if you try to attach more Databases than that.

By default, a Database attached this way uses the Overbase's own allocator for its tables. Pass one explicitly to override it:
...
```odin
ecs.add_entity(&positions, robot)   // world_ecs
ecs.add_entity(&sprites, robot)     // render_ecs
...
See [sample06](/samples/sample06/main.odin) for a complete two-Database save/restore example.

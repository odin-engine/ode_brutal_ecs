...
Fat Struct vs. ECS discussion is hot but I think that Fat Struct is just a fat component. You control granularity of your components. No one is telling you that you must extract position data into one component and velocity data into another component. You can unite them and other data into a fat component and still use ODE_BRUTAL_ECS for its other benefits without any disadvantages.
...
Using Fat Struct + ODE_BRUTAL_ECS, you can get benefits like:
...
- easily slicing over entities (or entities + `Inventory` + `AI_State`, or other combinations) to make your code faster.
- if you decide to, easily adding more components to entities — for example, `Equipped_Gear` or `Abilities` — dynamically, with O(1) speed in ODE_BRUTAL_ECS and minimal code.
...
A working example of Fat Struct as a fat component + ODE_BRUTAL_ECS is in [/samples/fat_struct/fat_component/main.odin](/samples/fat_struct/fat_component/main.odin).
...
I would probably create separate ODE_BRUTAL_ECS databases for separate entity types. For example, players and NPCs share many components, so I would put them in one database.
...
It's not that complicated when using ODE_BRUTAL_ECS, and it's easily scalable.
A working example of this idea is in [/samples/fat_struct/rpg/main.odin](/samples/fat_struct/rpg/main.odin).

Do we need a separate database for Item_Instances or an ecs.Table, for example? It depends on your game. Maybe, if you really want to track the same item between inventories and places in the world.

Bottom line for me is: don't be shy to use multiple ODE_BRUTAL_ECS databases to modularize entities.

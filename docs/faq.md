...
- Data-parallel iteration is already a designed-in feature: `slice(&units, T)`/`slice(&units)` hand back plain slices, and splitting a range into batches for worker threads is just index math over them.
...
In ECS, systems (essentially procedures) iterate over their related components. For instance, the network system processes network components, while the physics system handles physics components.

### 3. Which is the better pattern: Fat Struct or ECS?
---

Fat Struct is just a fat component. You can do Fat Struct + ODE_BRUTAL_ECS to get many benefits.
